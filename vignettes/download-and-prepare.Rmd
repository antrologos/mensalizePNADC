...
**Why stack multiple quarters?** The mensalization algorithm identifies reference months by tracking households across their panel interviews. With a single quarter, the determination rate is only ~70%. By stacking multiple quarters, the algorithm can leverage the rotating panel structure to achieve **over 97% determination**.

| Quarters Stacked | Month % | Fortnight % | Week % | Source |
|------------------|---------|-------------|--------|--------|
| 1 (single quarter) | 70% | 7% | 2% | Test data (2019-Q1) |
| 8 (2 years) | 94% | 9% | 3% | 2019-2020 benchmark |
| 20 (5 years) | 95% | 8% | 3% | 2020-2024 example |
| 55+ (full history) | **97%** | **9%** | **3%** | 2012-2025 complete data |

**Note:** Determination rates vary by time period due to data quality. The 2020-2024 period shows slightly lower rates due to COVID-19 collection disruptions in 2020.
...
# Note: If downloading more recent years, filter out quarters not yet available:
# editions <- editions |> filter(!(year == 2025 & quarter > 3))

# View the grid
editions
```

This creates a grid of 20 quarters. If extending to more recent years, adjust the filter for the latest available quarter.
...
Phase 1: Identifying MONTHS (all observations)...
  ...
  -> Month determination: 94.6% (8,386,003 of 8,868,247 observations)

Phase 2: Identifying FORTNIGHTS (within determined months)...
  Processing 8,386,003 observations with determined months...
  ...
  -> Fortnight determination: 8.4% (748,904 of 8,868,247 observations)

Phase 3: Identifying WEEKS (within determined fortnights)...
  Processing 748,904 observations with determined fortnights...
  Step 3.2: Aggregating at household level (within quarter)...
  Step 3.4: Determining reference weeks...
  -> Week determination: 3.0% (269,288 of 8,868,247 observations)
...
The determination rate tells you what percentage of observations have their reference period identified. With 20 quarters stacked (2020-2024), we achieve 94.6% for months, 8.4% for fortnights, and 3.0% for weeks.
...
The distribution is approximately equal across months 1, 2, and 3 (each around 31-32%), with 5.4% having `NA` for indeterminate cases. The 94.6% determination rate for this 2020-2024 subset is slightly lower than the 97.0% achieved with full 2012-2025 data due to:

- **Boundary effects**: The first and last 4 quarters in any dataset have incomplete panel coverage
- **COVID-19 impact**: 2020 data shows degraded determination rates due to IBGE collection disruptions
- **Panel accumulation**: More years of data = more panel cycles = higher cross-quarter determination
...
| `weight_monthly` | Monthly survey weight (if `calibrate = TRUE`) |
...
The full history provides approximately 29 million observations and achieves the highest possible determination rate.
...
4. **SIDRA API errors**: The weight calibration requires internet access to the SIDRA API. If it fails, try again later or use `calibrate = FALSE` for just reference period identification without weight calibration.
...
- Visit the [package website](https://antrologos.github.io/PNADCperiods/) for more resources
...
For questions or issues, please open an issue on [GitHub](https://github.com/antrologos/PNADCperiods/issues).
