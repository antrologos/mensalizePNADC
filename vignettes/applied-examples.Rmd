...
|-------------|-------------------|-------------------|----------|
| Monthly | ~97% | ~65,000 | Most analyses |
| Fortnightly | ~6% | ~3,000 | Transition timing |
| Weekly | ~1.5% | ~500 | Shock detection |
...
The strict determination rates above can be significantly improved using `pnadc_experimental_periods()`. This function applies two strategies:

1. **Probabilistic assignment** (`confidence_threshold = 0.9`): When the interview window spans exactly two consecutive periods and ≥90% of the interval falls in one period, assign to that period.

2. **UPA aggregation** (`upa_proportion_threshold = 0.7`): When ≥70% of strictly identified observations within a UPA have the same determined period, extend that assignment to undetermined observations. This leverages the empirical finding of 100% UPA homogeneity.

With both strategies (`strategy = "both"`):
| Granularity | Strict Rate | Experimental Rate | Improvement |
|-------------|-------------|-------------------|-------------|
| Fortnight | ~6% | ~27-28% | ~4-5× more data |
| Weekly | ~1.5% | ~2% | ~1.3× more data |

The examples below use experimental strategies to maximize sample sizes for sub-monthly analysis.
...
```{r covid-fortnight}
# Apply experimental strategies to boost fortnight determination rate
# Thresholds: 90% confidence for probabilistic, 70% UPA proportion for aggregation
crosswalk_exp <- pnadc_experimental_periods(
  crosswalk = crosswalk,
  data = pnadc,
  strategy = "both",  # probabilistic + UPA aggregation
  confidence_threshold = 0.9,       # >=90% of interval in one period
  upa_proportion_threshold = 0.7,   # >=70% UPA consensus for extension
  include_derived = TRUE
)

# The experimental crosswalk now has ~27-28% fortnight determination vs ~6% strict
...
ggplot() +
  ...
  labs(
    ...
    caption = "Uses pnadc_experimental_periods(strategy='both', confidence=0.9, upa_prop=0.7) for ~27-28% fortnight rate."
  ) +
  theme_minimal(base_size = 11)
```
...
ggplot(weekly_carnival,
  ...
  labs(
    ...
    fill = "Region",
    caption = "Uses pnadc_experimental_periods(strategy='both', confidence=0.9, upa_prop=0.7) for ~2% weekly rate. Data pooled across 5 years."
  ) +
  theme_minimal(base_size = 11)
```
...
1. **Monthly analysis handles most use cases** with ~97% strict determination rate
2. **Fortnightly analysis** is practical with `pnadc_experimental_periods()` (~27-28% rate vs ~6% strict)
3. **Weekly analysis** benefits from experimental strategies (~2% vs ~1.5% strict); pooling years essential for subgroup analysis
