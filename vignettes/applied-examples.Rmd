...
| Fortnightly | ~3% | ~3,000 | Transition timing |
| Weekly | ~1% | ~500 | Shock detection |
...
Carnival is Brazil's largest cultural event, concentrated in specific states (Bahia, Rio de Janeiro, Pernambuco) and occurring during a specific week each year. Does formalization rate change during Carnival week, as temporary and informal workers increase?

This is a challenging analysis due to the ~1% weekly determination rate. To increase sample size, we **pool multiple years** (2019, 2022-2024, excluding COVID years) and compare Carnival states to other states:
...
The comparison shows whether Carnival-intensive states experience a formalization dip during Carnival week that other states don't. Even with pooled data, the confidence intervals are wide due to the low weekly determination rate---but the methodology demonstrates how weekly analysis can be used for event studies when the effect is expected to be localized in time and space.

### Practical Guidelines for Sub-Monthly Analysis

| Question Type | Recommended Granularity | Sample Strategy |
|---------------|------------------------|-----------------|
| "How much changed?" | Monthly | Standard |
| "When exactly did it change?" | Fortnight | National-level only |
| "Was there a within-week pattern?" | Weekly | Pool multiple years |

**Key takeaways:**

1. **Monthly analysis handles most use cases** with high determination rates and robust samples
2. **Fortnightly analysis** works for detecting transition timing when the fortnight boundary aligns with the event of interest (e.g., mid-month policy changes)
3. **Weekly analysis** requires pooling multiple years and focusing on aggregate patterns; subgroup analysis is rarely feasible
