...
# Main income measure: per capita household income
# NOTE: VD4048 already includes all income sources (work, transfers, other)
# so we use it directly rather than adding VD4019 which would double-count work income
d[, rendaDom_mista_pc := rendaDom_base_pc]

# Counterfactual income (without emergency aid during 2020-2021)
# Subtracts "other programs" (v5003a2 - includes Auxílio Emergencial) and Bolsa Família
...
Does mensalization introduce systematic bias? We can check by comparing mensalized monthly estimates (aggregated to annual) against direct annual estimates computed from the same PNADC annual data using original survey weights:

```{r validation}
# 1. Aggregate monthly estimates to annual
...
# 2. Compute direct annual estimates (no mensalization)
d_annual_valid <- d[!is.na(rendaDom_mista_pc) &
                    !is.na(v1032) &
                    rendaDom_mista_pc >= 0]

annual_direct <- d_annual_valid[, .(
  direct_estimate = fgt(
    x = rendaDom_mista_pc,
    z = linhaPobreza_WB_media,
    w = v1032,
    alpha = 0
  )
), by = ano]
...
# 3. Compare
validation_data <- merge(annual_from_monthly, annual_direct, by = "ano")
```

![Validation: Mensalized vs Direct Annual Estimates](figures/annual-poverty-analysis/fig-validation.png){width=100%}

The high correlation and tight clustering around the diagonal confirm that mensalization preserves annual-level estimates while enabling monthly disaggregation. Small discrepancies are expected due to:
...
- **Averaging method**: Monthly series averaged vs. single annual calculation

For most research purposes, discrepancies under 2% relative difference are acceptable.
